...
function getSinglePerson(array, criteriaToMeet, property) {
  	var result = array[0] ? array[0] : null;

	for (var obj in data) {
      	if (criteriaToMeet(result[property], data[obj][property])) {
        	result = data[obj]
        }
  	}
...
var lowest = function(currentValue, valueToTest){
	return valueToTest < currentValue
}

var highest = function(currentValue, valueToTest){
	return valueToTest > currentValue
}

console.log(getSinglePerson(data, lowest, 'born'));


// Composability

// Code that finds the average age for men and for women in the data set.

function getAge(person) {
	return person.died - person.born;
}

function averageAge(sex, data) {
  	var allPeople = data.filter( function(person) { return person.sex == sex; });
  
  	var ages = allPeople.map(getAge);
